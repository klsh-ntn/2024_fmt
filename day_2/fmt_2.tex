...
\newcommand{\easymath}{
Директор КЛШ-47 Лиза М. носит командирские часы с 12-часовым циферблатом. 
Сколько раз в течение суток часовая и минутная стрелки образуют прямой угол на командирских часах Лизы М.?
}


\newcommand{\soleasymath}{
За сутки минутная стрелка делает 24 оборота, а часовая — два. 
Значит, минутная стрелка обгоняет часовую 22 раза. 
В момент обгона они совпадают. 
При этом одна полночь учитывается, а вторая — нет.
Моменты совпадения стрелок разбивают сутки на 22 равных отрезка. 
На каждом таком отрезке стрелки два раза перпендикулярны. Ответ: 44.    
}


\newcommand{\hardmath}{
Трапеция $ABCD$ — равнобедренная. 
Диагональ $AC$ длины $a$ образует угол $60^{\circ}$ с основанием $AD$. 

Найди длину средней линии трапеции. 
}
\newcommand{\solhardmath}{
Обозначим проекцию точки $C$ на сторону $AD$ обозначим как $C'$. 
Угол $ACC'$ равен $30^{\circ}$, следовательно, $AC' = a/2$.
Остается заметить, что средняя линия равна $AC'$. Ответ: $a/2$.
}
...
\newcommand{\easyphys}{
В безопасный летнешкольный электрический чайник мощностью $600$ Вт и КПД $50\%$ налили $2$ литра воды с начальной температурой $20^{\circ}$.

Через сколько секунд после включения чайника вода закипит? 

Удельная теплоёмкость воды равна $4200$ Дж/(кг$\cdot ^{\circ}C$). 
}

\newcommand{\soleasyphys}{
$Q = 4200 \cdot 2 \cdot (100 - 20) = 672000$, $A = 672000 / 0.5 = 1344000$, 
$t = 1344000 / 600 = 2240$ секунд (кстати, это 37 минут и 20 секунд). 
}
...
\newcommand{\hardphys}{
Кубик плотности $\rho_0$ с длиной ребра $a$ плавает в супе с плотностью $\rho_1$. 
Поверх супа в кастрюлю наливают компот с плотностью $\rho_2$ вровень с верхней гранью кубика. 
    
Какова высота слоя второй жидкости, если суп и компот не смешиваются и $\rho_1>\rho_2$?
}
\newcommand{\solhardphys}{
Разобъём высоту кубика на две части, $a = h_1 + h_2$. 
\[
P^* = \rho_1 g h_1 + \rho_2 g h_2 + P_0
\]    
Из баланса сил получаем второе уравнение:
\[
F = (P^* - P_0) S = mg \quad \Rightarrow \quad (\rho_1 gh + \rho_2 gh)S = \rho_0 Sh g
\]
Получаем систему и находим $h_2$ из неё:
\[
\begin{cases}
    h_1 + h_2 = a \\
    \rho_1 h_1 + \rho_2 h_2 = \rho_0 a \\
\end{cases} \quad \Rightarrow \quad h_2 = \frac{\rho_1 - \rho_0}{\rho_1 - \rho_2}a
\]
}
